...
curl_setopt($curl, CURLOPT_URL, 'http://10.33.35.38:8080/api_intero/?endpoint=appointment_history');
$res = curl_exec($curl);
                ...
                <tbody id="reservationHistoryTable">
                    ...
                    for ($i = 0; $i < count($json); $i++){
                        echo "<tr>";
                            ...
                            echo "<td> {$json[$i]["reservation_date"]} </td>";
                            echo "<td> {$json[$i]["reservation_time"]} </td>";
                            echo "<td> {$json[$i]["reservation_status"]} </td>";
                        echo "</tr>";
                    }
                    ?>
                </tbody>
